...
partita_scelta = st.selectbox(
    ...
    options=df_partite['id'].tolist(),
    format_func=lambda x: f"{x} - {df_partite[df_partite['id'] == x]['avversario'].values[0]}"
)
...
# -- 5. Calcolo minutaggi e quartetti --
df['quartetto_tot'] = df.apply(get_giocatori_in_campo, axis=1)
minutaggi = calcola_minutaggi(df)

# -- 6. Durata partita --
durata_primo = differenza_tempi(t_start, t_fine_primo)
durata_secondo = differenza_tempi(t_start_secondo, t_fine_partita)
...
risultato = f"{len(gol_fatti)} - {len(gol_subiti)}"

st.header(f"Partita: {partita_scelta}")
st.subheader("Risultato")
st.markdown(f"**Noi {risultato} Loro**")

# -- 8. Durata effettiva --
st.subheader("Durata effettiva")
st.dataframe(dati_durate, use_container_width=True)
...
with col2:
    ...
    st.dataframe(minutaggi['quartetti_1t'], use_container_width=True)
with col3:
    st.markdown("**Secondo tempo**")
    st.dataframe(minutaggi['quartetti_2t'], use_container_width=True)

# -- 11. Minutaggi Singoli --
st.subheader("Minutaggi Singoli")
...
with col5:
    st.markdown("**Primo tempo**")
    st.dataframe(minutaggi['singoli_1t'], use_container_width=True)
with col6:
    st.markdown("**Secondo tempo**")
    st.dataframe(minutaggi['singoli_2t'], use_container_width=True)

# -- 12. Minutaggi Coppie --
st.subheader("Minutaggi Coppie")
col7, col8, col9 = st.columns(3)
with col7:
    st.markdown("**Totale**")
    st.dataframe(minutaggi['coppie_tot'], use_container_width=True)
with col8:
    st.markdown("**Primo tempo**")
    st.dataframe(minutaggi['coppie_1t'], use_container_width=True)
with col9:
    st.markdown("**Secondo tempo**")
    st.dataframe(minutaggi['coppie_2t'], use_container_width=True)


# -- 12. Eventi per tempo (tabella dettagliata) --
...
for evento in eventi_da_contare:
    fatti_primo = len(df_primo[(df_primo['evento'] == evento) & (df_primo['squadra'] != 'Loro')])
    subiti_primo = len(df_primo[(df_primo['evento'] == evento) & (df_primo['squadra'] == 'Loro')])
    fatti_secondo = len(df_secondo[(df_secondo['evento'] == evento) & (df_secondo['squadra'] != 'Loro')])
    subiti_secondo = len(df_secondo[(df_secondo['evento'] == evento) & (df_secondo['squadra'] == 'Loro')])
...
st.dataframe(df_eventi_divisi, use_container_width=True)

# -- 13. Grafico tiri per zona con funzione dedicata --
st.subheader("Tiri per Zona")
fig = plot_tiri_per_zona(df, squadra='Noi')
st.pyplot(fig)

# -- 13. Grafico tiri per zona con funzione dedicata --
st.subheader("Tiri subiti per Zona")
fig = plot_tiri_per_zona(df, squadra='Loro')
st.pyplot(fig)